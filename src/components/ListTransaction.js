const ListTransaction = (props) => {
    ...
    const getTanggal = (tanggal) => {
      ...
      const bulan = arrBulan[dateObj.getMonth()];
      return `${dateObj.getDate()} ${bulan} ${dateObj.getFullYear()}`;
    };
  
    // Untuk menentukan style list transaction (warna border kiri)
    const classTransaction = props.nominal > 0 ? "list-pemasukan" : "list-pengeluaran";
  
    return (
      <div className={`list-transaction p-1 mb-2 list-pemasukan ${classTransaction}`}>
